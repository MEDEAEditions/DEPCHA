...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" version="2.0"
    ...
    <!--<xsl:include href="https://gams.uni-graz.at/gamsdev/pollin/medea/trunk/www/medea-templates.xsl"/>-->
    <!-- VARIABLES -->   
    <!-- //////////////////////////////// -->
    <!-- global Variables -->
    ...
    <xsl:template match="t:text//*[tokenize(@ana, ' ') = '#bk_entry']">
        ...
        <!-- concat('https://gams.uni-graz.at/', $Actual_PID, '#Entry-', $Position, '-', .//.[tokenize(@ana, ' ') = '#bk_to']/@ref) -->
        ...
        <!-- /////////// -->
        <!-- TRANSACTION -->
        <xsl:variable name="Transaction-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID, '#Transaction-',  $Position)"/>
        ...
        <bk:Transaction rdf:about="{$Transaction-ID}">
            ...
            <!--<xsl:for-each-group select="$bk_Measurables" group-by="@ana">
                    <xsl:variable name="Transfer-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID, '#Entry-', $Position, '-Transfer-', position())"/>
                    <bk:consists-of rdf:resource="{$Transfer-ID}"/>
            </xsl:for-each-group>-->
            <xsl:for-each-group select="$bk_Measurables" group-by="@ana">
                <xsl:variable name="Transfer-ID" select="concat($Transaction-ID, '-Transfer-', position())"/>
           ...
           <!-- connect every Transaction to a Collection (like context:medea.wheaton) -->
            ...
            <!-- partOfTEI -->
            <gams:isPartofTEI rdf:resource="{concat('https://glossa.uni-graz.at/', $Actual_PID)}"/>
        ...
        <xsl:for-each-group select="$bk_Measurables" group-by="@ana">
            <xsl:variable name="Transfer-ID" select="concat($Transaction-ID, '-Transfer-', position())"/>
            <xsl:variable name="Measurable-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-',  $Position, '-Measurable-', position())"/>
            <bk:Transfer rdf:about="{$Transfer-ID}">  
               ...
               <!-- <xsl:for-each select="current-group()">
                ...
                <xsl:if test="contains(current-grouping-key(), '#bk_agent')">
                    <bk:by rdf:resource="{concat('https://gams.uni-graz.at/', $Actual_PID, '#Entry-',  $Position, '-', $bk_Agent-ID)}"/>
                </xsl:if>
                            ...
                            <xsl:for-each select="current-group()">
                                <xsl:element name="{concat('bk:', $SubClassofMeasurables)}">
                                    ...
                                    <bk:text>
                                        ...
                                        <xsl:variable name="Price-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-Price-', position())"/>
        ...
        <xsl:for-each-group select="$bk_Agent" group-by=".">
            <bk:Agent rdf:about="{concat('https://gams.uni-graz.at/', $Actual_PID, '#Entry-',  $Position, '-', $bk_Agent-ID)}">
                    <bk:name>
...
<!--  
        <xsl:variable name="Entry-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID, '#Entry-', $Position)"/>-->

<!-- Measurables -->
<!--  <xsl:variable name="Good-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-',  $Position, '-Good')"/>
        <xsl:variable name="MonetaryValue-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-MonetaryValue')"/>
        <xsl:variable name="Service-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-Service')"/>
        <xsl:variable name="Price-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-Price')"/>
        
        <xsl:variable name="Between-ID" select="concat('https://gams.uni-graz.at/', '-Between-')"/>-->
<!--<xsl:variable name="Party-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-Party')"/>
        <xsl:variable name="Account-ID" select="concat('https://gams.uni-graz.at/', $Actual_PID,  '#Entry-', $Position, '-Account')"/>-->
